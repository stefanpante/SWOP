...
public class TestForceFieldManager {
	...
	private Grid getGridWith(ArrayList<Coordinate> forceFieldsGen) {
		...
		Grid grid =  GridProvider.getGrid(10, 10, walls);

		for(Coordinate c : forceFieldsGen){
			if(grid.getGridElement(c) instanceof Square){
				Square s = (Square) grid.getGridElement(c);
				ForceFieldGenerator f = (ForceFieldGenerator) s.filterItemsByType((new ForceFieldGenerator()).get(0);
				f.activate();
			}
		}
